...
contract("NftContract",  function (accounts) {
  ...
  it("should revert mint value", async function () {
    ...
    await nftContract.mint(1,1,{ from: accounts[1],value:await nftContract.getTokenPrice(1) }).should.be.rejectedWith(EVM_REVERT+"The address isn't authorized -- Reason given: The address isn't authorized.");
  });
  it("should mint token when user is added", async function () {
    ...
    await nftContract.mint(1,1,{ from: accounts[1],value:await nftContract.getTokenPrice(1) });
  });
  it("should not mint excess tokens", async function () {
    let nftContract=await NftContract.deployed();
    let res = await nftContract.addPrivateSaleBuyer(accounts[1],{ from: accounts[0]});
    await nftContract.mint(1,1,{ from: accounts[1],value:await nftContract.getTokenPrice(1) }).should.be.rejectedWith(EVM_REVERT+"Number of tokens minted exceed maximum set for private sale");
  });

  it("should get private mint price within 24hrs", async function () {
    let nftContract=await NftContract.deployed();
    let privateMintPrice=await nftContract.getTokenPrice(1);
  ...
  it("should get public mint price past 24hrs", async function () {
    ...
    let nftContract=await NftContract.deployed();
    let publicMintPrice=await nftContract.getTokenPrice(1);
